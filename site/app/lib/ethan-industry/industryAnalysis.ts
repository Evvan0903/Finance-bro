...
function emptyCoverage(status: EthanIndustryCoverage["status"]): EthanIndustryCoverage {
  return {
    status,
    overallStatus: "Insufficient industry data",
    directOfficialMetricCount: 0,
    proxyMetricCount: 0,
...
function disabledCoverage(): EthanIndustryCoverage {
  return emptyCoverage("disabled");
}

function unavailableCoverage(): EthanIndustryCoverage {
  return emptyCoverage("unavailable");
}

function observationPeriod(report: MarketReport) {
  const periods = [...report.dataCoverage.timePeriodsCovered].sort();
...
function coverageFrom(report: MarketReport): EthanIndustryCoverage {
  ...
  const directOfficialMetricCount = report.metrics.filter(
    (metric) => !metric.isProxy && !metric.isCalculated,
  ).length;
  ...
  const overallStatus: EthanIndustryCoverage["overallStatus"] = report.metrics.length === 0
    ...
    : directOfficialMetricCount === 0
      ...
      : unavailable.length === 0 && proxyMetricCount === 0
        ? "Strong official-data coverage"
        : "Partial official-data coverage";
  const dataRetrievalDate = report.providerResults
  ...
  return {
    ...
    proxyMetricCount,
    observationPeriod: observationPeriod(report),
    dataRetrievalDate,
  };
}
