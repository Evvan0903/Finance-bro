...
export function ClaraFundingTimeline({ data, locale, mode = 'visualization' }: { data: FundingTimelineData; locale: DiligenceLocale; mode?: PresentationMode }) {
  const zh = locale === 'zh';
  const groups = [
  ...
  return <article data-mode={mode} className={`clara-visual-card clara-funding-timeline${mode === 'visualization' ? ' is-timeline' : ' is-summary'}`} aria-label={zh ? '融资时间线' : 'Funding timeline'}>
    <header><h3>{mode === 'visualization' ? zh ? '融资时间线' : 'Funding Timeline' : zh ? '融资摘要' : 'Financing Summary'}</h3><p>{zh ? '独立保留来源披露；不推断当前融资阶段' : 'Source observations; no current funding stage inferred'}</p></header>
    ...
    {groups.filter(g => g.items.length).map(group => <section key={group.id}><h4 className="clara-timeline-group-title">{group.title}</h4><ol className={mode === 'visualization' && group.id === 'financings' ? 'clara-funding-rail clara-horizontal-rail' : 'clara-funding-rail clara-summary-rail'}>{group.items.map(item => <Observation key={item.id} item={item} locale={locale} />)}</ol></section>)}
