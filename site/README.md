...
Set Vercel's Root Directory to `site`. The project uses npm only: `npm ci` and `npm run vercel-build` (`next build --webpack`). Configure server-only provider variables separately in Vercel Project Settings for each intended environment; local `.env.local` values are not uploaded. Ethan uses `SEC_USER_AGENT`, `FRED_API_KEY`, `BEA_API_KEY`, `CENSUS_API_KEY`, and `DATA_GOV_API_KEY` when applicable. Clara requires `TURSO_DATABASE_URL` and `TURSO_AUTH_TOKEN` in Vercel, uses `SERPAPI_API_KEY` and/or `TAVILY_API_KEY` for bounded post-confirmation web discovery (SerpApi first by default, with eligible Tavily fallback; optional `CLARA_SEARCH_PRIMARY=tavily`), and may use `DEEPSEEK_API_KEY` only to rank already-grounded candidate records. Never use `NEXT_PUBLIC_` for credentials. Changing a Vercel variable requires a new deployment before the runtime sees it.
...
**Outside-In Due Diligence** remains an in-development, deeper workflow at `/workflows/private-company-diligence`. The two modes share entity resolution, provider, evidence, claim, export, and security infrastructure but keep their report scopes separate. Clara is primarily deterministic: Wikidata/Wikipedia and website data discover candidates, and SerpApi leads are fetched as original pages before they can become evidence. When configured, DeepSeek is limited to structured ranking of those grounded candidates; it cannot create candidates or bypass evidence validation.
